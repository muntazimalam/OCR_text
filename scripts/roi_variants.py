...
from app.analyzers.ocr import _locate_plate_rois, _crop_and_enhance_patch, _run_engine
...
def main():
    ...
    for path in files:
        ...
        for idx, roi in enumerate(_locate_plate_rois(img, top_n=5)):
            x, y, w, h = roi
            patch = _crop_and_enhance_patch(img, roi)
            if patch is None:
            ...
            for name, var_img in variants(patch).items():
                res = _run_engine(reader, var_img, min_chars=2)
                txt = (res or {}).get("text", "")
